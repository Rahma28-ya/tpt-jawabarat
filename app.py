...
Algoritma: Random Forest Regressor — Skenario 80/20
"""
...
st.markdown(
    f"""
<style>
.stApp {{ background: linear-gradient(160deg, {C_BG} 0%, #F3E8FF 45%, #EAF4FF 100%); }}
h1, h2, h3 {{ color: {C_TEXT}; font-weight: 800; }}
.hero-box {{
    background: linear-gradient(120deg, {C_PINK_SOFT} 0%, {C_PURPLE_SOFT} 55%, {C_BLUE_SOFT} 100%);
    padding: 1.6rem 2rem; border-radius: 22px; margin-bottom: 1.4rem;
    box-shadow: 0 8px 24px rgba(166,108,255,0.15);
}}
.metric-card {{
    background: white; border-radius: 18px; padding: 1.1rem 1.3rem;
    box-shadow: 0 6px 18px rgba(166,108,255,0.12); border: 1px solid #F0DFFF;
}}
.metric-card h3 {{
    margin:0; font-size:0.85rem; color:#8B7AA8; font-weight:600;
    text-transform:uppercase; letter-spacing:0.04em;
}}
.metric-card .big {{ font-size:1.9rem; font-weight:800; margin:0.15rem 0; }}
.metric-card .sub {{ font-size:0.85rem; color:#6B5C85; }}
section[data-testid="stSidebar"] {{ background: linear-gradient(180deg,#FBEAF5 0%,#EFE3FF 100%); }}
div[data-testid="stMetric"] {{
    background:white; padding:0.9rem 1rem; border-radius:16px;
    box-shadow:0 4px 14px rgba(166,108,255,0.1);
}}
.footer-note {{ color:#8B7AA8; font-size:0.8rem; text-align:center; margin-top:2rem; }}
</style>
""",
...
@st.cache_data
def load_split_80_20():
    train = pd.read_csv(DATA_DIR / "data_train_80_20.csv")
    test  = pd.read_csv(DATA_DIR / "data_test_80_20.csv")
    return train, test
...
@st.cache_resource
def train_evaluate():
    """Melatih Random Forest pada skenario 80/20 dan mengevaluasinya."""
    train, test = load_split_80_20()
    ...
    df_pred = test.copy()
    df_pred["Prediksi_RF"] = pred_rf
    df_pred = df_pred.sort_values(["Kabupaten_Kota", "Tahun"]).reset_index(drop=True)
    df_pred.attrs["rf_importance"] = dict(zip(FEATURES, rf.feature_importances_))

    return df_eval, df_pred

@st.cache_resource
def train_full_model():
    """Melatih model RF pada SELURUH data untuk prediksi langsung."""
    df = load_integrated()
    rf = RandomForestRegressor(n_estimators=500, random_state=42)
...
with st.sidebar:
    ...
    menu = option_menu(
        menu_title=None,
        options=["Beranda","Peta Jawa Barat","Tren Provinsi",
                 "Aktual vs Prediksi","Evaluasi Model","Prediksi TPT","Data Lengkap"],
        icons=["house-heart","geo-alt","graph-up-arrow","bullseye",
               "clipboard-data","magic","table"],
        default_index=0,
        styles={
            "container": {"padding":"0","background-color":"transparent"},
            "icon": {"color":C_PURPLE,"font-size":"16px"},
            "nav-link": {"font-size":"14px","text-align":"left","margin":"3px 0",
                         "border-radius":"12px","--hover-color":C_PURPLE_SOFT},
    ...
    st.markdown(
        ...
        <div class="big" style="font-size:1.05rem; color:{C_PURPLE};">Random Forest Regressor</div>
        <div class="sub">Skenario {SKENARIO_LABEL} &nbsp;|&nbsp; R² = {eval_row['R2']:.3f}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if menu == "Beranda":
    st.markdown(
        ...
        <div class="hero-box">
            ...
            <b>Random Forest Regressor</b> dengan skenario pembagian data <b>80/20</b>.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.caption(
        f"ℹ️ TPT Provinsi dihitung tertimbang sesuai metode resmi BPS — **{rata2:.2f}%**. "
        f"Rata-rata sederhana antar 27 kab/kota: **{rata2_sederhana:.2f}%**."
    )
    ...
    with col_b:
        ...
        fig2.update_layout(yaxis={"categoryorder":"total descending"}, height=350,
                           showlegend=False, plot_bgcolor="rgba(0,0,0,0)",
                           paper_bgcolor="rgba(0,0,0,0)", coloraxis_showscale=False,
                           margin=dict(l=0,r=10,t=10,b=0))
    ...
    st.info(
        f"💡 **Random Forest Regressor** — Skenario {SKENARIO_LABEL}: "
        f"R²={eval_row['R2']:.3f}, MAE={eval_row['MAE']:.3f}, "
        f"RMSE={eval_row['RMSE']:.3f}. Lihat detail di menu **Evaluasi Model**."
    )
...
# ── HALAMAN: PETA JAWA BARAT ─────────────────────────────────────────────────
    ...
    fig_map = px.choropleth(
        df_map, geojson=geojson_jabar, locations="Kabupaten_Kota",
        featureidkey="properties.Kabupaten_Kota", color="TPT",
...
# ── HALAMAN: TREN PROVINSI ───────────────────────────────────────────────────
    ...
    st.caption("📐 Garis **Rata-rata Provinsi** dihitung tertimbang berdasarkan Jumlah Angkatan Kerja (metode BPS).")
    ...
    fig.update_layout(height=440, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
    ...
    wilayah_pilih   = st.multiselect("Pilih kabupaten/kota:", sorted(df_all["Kabupaten_Kota"].unique()),
...
# ── HALAMAN: AKTUAL VS PREDIKSI ──────────────────────────────────────────────
    ...
    df_show = df_pred if tahun_pilih_pred == "Semua Tahun" \
              else df_pred[df_pred["Tahun"] == tahun_pilih_pred]
    ...
    df_melt = df_show.melt(id_vars=["Kabupaten_Kota","Tahun"],
                            value_vars=["TPT","Prediksi_RF"],
                            var_name="Jenis", value_name="Nilai")
    df_melt["Jenis"] = df_melt["Jenis"].replace({"TPT":"Aktual","Prediksi_RF":"Prediksi (RF)"})
    fig = px.bar(df_melt.sort_values(["Kabupaten_Kota","Tahun"]),
                 x="Kabupaten_Kota", y="Nilai", color="Jenis", barmode="group",
                 color_discrete_map={"Aktual":C_PURPLE,"Prediksi (RF)":C_PINK}, height=480)
    fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                      xaxis_tickangle=-45, legend_title="", margin=dict(l=0,r=0,t=10,b=0))
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("##### 🔵 Scatter Aktual vs Prediksi")
    fig_sc = px.scatter(df_show, x="TPT", y="Prediksi_RF", color_discrete_sequence=[C_PURPLE],
                         hover_name="Kabupaten_Kota",
                         labels={"TPT":"Aktual","Prediksi_RF":"Prediksi RF"})
    ...
    tabel_show = df_show[["Kabupaten_Kota","Tahun","TPT","Prediksi_RF","Error_RF"]].copy()
    tabel_show.columns = ["Kabupaten/Kota","Tahun","Aktual","Prediksi RF","Error RF"]
    st.dataframe(
        tabel_show.style.format({"Aktual":"{:.2f}","Prediksi RF":"{:.2f}","Error RF":"{:.2f}"})
...
# ── HALAMAN: EVALUASI MODEL ──────────────────────────────────────────────────
    ...
    st.markdown("## 📐 Evaluasi Model — Random Forest Regressor")
    st.caption(f"Hasil evaluasi pada skenario pembagian data **{SKENARIO_LABEL}** (train 80% / test 20%).")
    ...
    metrik_pilih = st.radio("Pilih metrik:", ["MAE","RMSE","R2"], horizontal=True)
    fig_m = px.bar(
        df_eval, x="Model", y=metrik_pilih,
        color_discrete_sequence=[C_PURPLE], text=metrik_pilih, height=360,
    )
    ...
    st.success(
        f"✅ **Random Forest Regressor** — Skenario {SKENARIO_LABEL}: "
        f"MAE={eval_row['MAE']:.4f}, RMSE={eval_row['RMSE']:.4f}, R²={eval_row['R2']:.4f}"
    )
    ...
    if imp_dict:
        ...
        fig_imp = px.bar(df_imp.sort_values("Importance"), x="Importance", y="Fitur",
                          orientation="h", color="Importance",
                          color_continuous_scale=SEQ_COLORSCALE)
        fig_imp.update_layout(height=320, plot_bgcolor="rgba(0,0,0,0)",
                               paper_bgcolor="rgba(0,0,0,0)",
...
# ── HALAMAN: PREDIKSI TPT ────────────────────────────────────────────────────
    ...
    with tab1:
        ...
        with st.form("form_prediksi"):
            ...
            with colp1:
                jumlah_penduduk = st.number_input(FEATURE_LABEL["Jumlah_Penduduk"],
        ...
        if submit:
            ...
            st.markdown(
                f"""<div class="hero-box" style="text-align:center;">
                    ...
                    <div style="font-size:0.85rem; color:#8B7AA8;">Random Forest Regressor</div>
    ...
    with tab2:
        ...
        if "df_2026" in st.session_state:
            ...
            st.write("")
            fig26 = px.bar(df_2026, x="TPT_2026", y="Kabupaten_Kota", orientation="h",
                            color="TPT_2026", color_continuous_scale=SEQ_COLORSCALE)
            fig26.update_layout(height=650, yaxis={"categoryorder":"total ascending"},
            ...
            fig_map26.update_layout(height=500, margin=dict(l=0,r=0,t=10,b=0),
                                     paper_bgcolor="rgba(0,0,0,0)")
            ...
            st.dataframe(
                tampil.style.format({"Jumlah Penduduk":"{:,.0f}","TPAK":"{:.2f}",
                                     "IPM":"{:.2f}","PDRB":"{:,.0f}","Prediksi TPT 2026":"{:.2f}"})
                .background_gradient(cmap="RdPu", subset=["Prediksi TPT 2026"]),
                use_container_width=True, height=420,
            )
            st.caption("⚠️ Proyeksi 2026 bersifat estimasi berbasis tren historis, bukan data resmi BPS.")
        else:
...
# ── HALAMAN: DATA LENGKAP ─────────────────────────────────────────────────────
    ...
    with colf1:
        wilayah_filter = st.multiselect("Filter Kabupaten/Kota:",
                                         sorted(df_all["Kabupaten_Kota"].unique()))
    with colf2:
    ...
    st.dataframe(
        df_filtered.style.format(
            {"IPM":"{:.2f}","TPT":"{:.2f}","Jumlah_Penduduk":"{:,.0f}",
             "PDRB":"{:,.0f}","TPAK":"{:.2f}"}
...
st.markdown(
    ...
    Dashboard Prediksi TPT Provinsi Jawa Barat · Random Forest Regressor · Skenario 80/20
    </div>
    """,
    unsafe_allow_html=True,
)
